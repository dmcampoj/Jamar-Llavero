...
function enhanceGuide(codeGuide){var body=document.getElementById('guideDetailBodyV49');if(!body)return;var old=body.querySelector('#guideSalesSummary78');if(old)old.remove();body.querySelectorAll('.guideSalesExtra78').forEach(function(x){x.remove()});body.querySelectorAll('.guideSalesTable78').forEach(function(t){t.classList.remove('guideSalesTable78')});var codes=guideCodesFromBody(body),sm=salesMap(st()),totalU=0,totalV=0,withSale=0,zero=0,months=[0,0,0],top=null;codes.forEach(function(c){var s=sm[c]||{units:0,value:0},m=month(c);totalU+=s.units;totalV+=s.value;if(s.units>0||s.value>0)withSale++;else zero++;if(!top||s.units>top.units)top={c:c,units:s.units,name:prod(c).n||c};for(var i=0;i<3;i++)months[i]+=m.u[i]});var t=trend(months),pct=codes.length?withSale/codes.length*100:0,summary='<div class="guideSalesSummary78" id="guideSalesSummary78"><div class="guideSalesGrid78"><div class="guideSalesCard78"><div class="l">Venta últimos 3 meses</div><div class="v">'+fi(totalU)+' unidades</div><div class="m">'+fm(totalV)+' en la tienda</div></div><div class="guideSalesCard78"><div class="l">Productos con venta</div><div class="v">'+pct.toLocaleString('es-CO',{maximumFractionDigits:1})+'%</div><div class="m">'+fi(withSale)+' de '+fi(codes.length)+' productos</div></div><div class="guideSalesCard78"><div class="l">Comportamiento de la guía</div><div class="v"><span class="guideTrend78 '+t.cl+'">'+t.txt+'</span></div><div class="m">Julio frente a junio</div></div><div class="guideSalesCard78"><div class="l">Producto líder</div><div class="v">'+(top?fi(top.units)+' u':'0 u')+'</div><div class="m">'+h(top?top.name:'Sin ventas')+'</div></div><div class="guideSalesCard78"><div class="l">Sin venta 3 meses</div><div class="v">'+fi(zero)+' productos</div><div class="m">'+(codes.length?(zero/codes.length*100).toLocaleString('es-CO',{maximumFractionDigits:1}):'0')+'% de la guía</div></div></div><div class="guideSalesNote78"><b>Lectura:</b> el acumulado de tres meses corresponde a la tienda. La tendencia Mayo–Junio–Julio corresponde al comportamiento mensual del producto disponible en el corte.</div></div>';var stats=body.querySelector('.guideModalStatsV48');if(stats)stats.insertAdjacentHTML('afterend',summary);body.querySelectorAll('.guideFloorTableV50').forEach(function(table){table.classList.add('guideSalesTable78');var hr=table.querySelector('thead tr');if(hr)hr.insertAdjacentHTML('beforeend','<th class="guideSalesExtra78 num">Venta 3 meses</th><th class="guideSalesExtra78">Comportamiento mensual</th><th class="guideSalesExtra78">Variación</th><th class="guideSalesExtra78 num">Participación guía</th>');table.querySelectorAll('tbody tr[data-product-code]').forEach(function(tr){var c=code(tr.dataset.productCode),s=sm[c]||{units:0,value:0},m=month(c),tt=trend(m.u),share=totalU?s.units/totalU*100:0;tr.insertAdjacentHTML('beforeend','<td class="guideSalesExtra78 guideSalesCell78 num"><b>'+fi(s.units)+' u</b><span>'+fm(s.value)+'</span></td><td class="guideSalesExtra78">'+bars(m.u)+'</td><td class="guideSalesExtra78"><span class="guideTrend78 '+tt.cl+'">'+tt.txt+'</span></td><td class="guideSalesExtra78 num"><b>'+share.toLocaleString('es-CO',{maximumFractionDigits:1})+'%</b><div class="guideShare78"><i style="width:'+Math.min(100,share)+'%"></i></div></td>')})})}
